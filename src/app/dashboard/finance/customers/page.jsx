...
function AllTransactionsView() {
  ...
  const load = useCallback(async () => {
    setLoading(true); setErr('')
    try {
      const [invRes, recRes] = await Promise.all([customerInvoiceApi.list(), customerReceiptApi.list()])
  ...
  return (
    <div>
      ...
      {err && (
        <div style={{ padding:'12px 16px', borderRadius:10, background:'#FEF2F2', border:'1px solid #FCA5A5', color:'#DC2626', fontSize:13, marginBottom:16, display:'flex', gap:8, alignItems:'center' }}>
          <AlertCircle size={14}/> {err}
        </div>
      )}
...
export default function CustomersPage() {
  ...
  const [view,          setView]          = useState('customers') // 'customers' | 'transactions'
      ...
      <div className="cust-hero">
        <div className="cust-hero-top">
          ...
          <div className="cust-view-toggle">
            <button className={`cust-view-btn${view === 'customers' ? ' active' : ''}`} onClick={() => setView('customers')}>
              <Building2 size={13}/> Customers
            </button>
            <button className={`cust-view-btn${view === 'transactions' ? ' active' : ''}`} onClick={() => setView('transactions')}>
              <Layers size={13}/> All Invoices &amp; Receipts
            </button>
          </div>
          {view === 'customers' && (
